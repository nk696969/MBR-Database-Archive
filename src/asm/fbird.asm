...
fb21:   mov di,pipe     ; Init variables in video segment (saves big bytes)
        ...
        mov ax,0x0e46   ; 'F' in white, good old ASCII
        stosw
        mov al,0x42     ; 'B'
        stosw
        mov al,0x49     ; 'I'
...
fb16:   add al,[di+2]   ; Get character below head
        ...
        mov byte [di+2],$2a
        mov di,0x07CA   
        mov ax,0x0c44   ; 'D' in white, good old ASCII
        stosw
        mov al,0x45     ; 'E'
        stosw
        mov al,0x41     ; 'A'
...
fb25:   xor dx,dx       ; Extend AX to 32 bits
        ...
        div bx          ; Divide
        add dx,0x0f30   ; Convert remaining 0-9 to ASCII, also put color
        xchg ax,dx
        std
        stosw
...
fb2:    mov cx,79       ; 79 columns
        ...
        ; Insert houses
        ;
        mov word [0x0f9e],0x0adf        ; Terrain
        in al,(0x40)    ; Get "random" number
        and al,0x70
        jz fb5
        mov bx,0x04db   ; House of one floor
        mov [0x0efe],bx
        mov di,0x0e5e
        ...
        sub di,0x00a0
fb3:    mov word [di],0x041e ; Add roof
        ;
